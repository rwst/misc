...
EXAMPLES::
    ...
    sage: R.<x>=ZZ[]
    sage: fibo = CFS(x/(1-x-x^2))        # the Fibonacci sequence
    sage: fibo
...
EXAMPLES::
    ...
    sage: r = CFS.from_recurrence([1,1],[0,1])
    sage: r == fibo
    True
...
EXAMPLES::
    ...
    sage: r = CFS.guess([0,1,1,2,3,5,8]);
    sage: r == fibo
    True
...
class CFS(FractionFieldElement):
    ...
    The ogf is normalized to get a denominator constant coefficient of one::
        ...
        if isinstance (ogf, FractionFieldElement):
            ...
            super(CFS, self).__init__(P.fraction_field(), num, den, *args, **kwargs)
            ...
            super(CFS, self).__init__(ogf.parent().fraction_field(), P(ogf), 1, *args, **kwargs)
            self._c = []
            ...
            raise ValueError("Cannot convert a " + str(type(ogf)) + " to CFS.")
    ...
    EXAMPLES::
        ...
        sage: CFS.from_recurrence([1,1],[0,1])   # Fibonacci numbers
        C-finite sequence, generated by x/(-x^2 - x + 1)
        sage: CFS.from_recurrence([-1,2],[0,1])    # natural numbers
        C-finite sequence, generated by x/(x^2 - 2*x + 1)
        sage: r = CFS.from_recurrence([-1],[1])
        sage: s = CFS.from_recurrence([-1],[1,-1])
        sage: r == s
        True
        sage: r = CFS(x^3/(1-x-x^2))
        sage: s = CFS.from_recurrence([1,1],[0,0,0,1,1])
        sage: r == s
        True
        """
    ...
    EXAMPLES::
        ...
        sage: r[0:5]                  # a(n) = 2^n
        [1, 2, 4, 8, 16]
        sage: s = CFS.from_recurrence([1],[1])
        ...
        return CFS(self.ogf() + other.numerator()/other.denominator())
    ...
    def coefficients(self):
        ...
        EXAMPLES::
            ...
            sage: R.<x>=ZZ[]
            sage: lucas = CFS((2-x)/(1-x-x^2))        # the Lucas sequence
    ...
    EXAMPLES::
        ...
        sage: r = CFS.from_recurrence([1,1],[2,1])
        sage: s = CFS.from_recurrence([-1],[1])
        sage: r == s
        False
        sage: R.<x> = ZZ[]
    ...
    def __getitem__(self, key) :
        ...
        EXAMPLE::
            ...
            sage: r = CFS.from_recurrence([3,3],[2,1])
            sage: r[2]
            9
            sage: r[101]
            ...
            sage: r[66]
            0
            sage: lucas = CFS.from_recurrence([1,1],[2,1])
            sage: lucas[5:10]
            [11, 18, 29, 47, 76]
            sage: r = CFS((2-x)/x/(1-x-x*x))
            sage: r[0:4]
            [1, 3, 4, 7]
            sage: r = CFS(1-2*x^2)
            sage: r[0:4]
            ...
            [0, 1, 0 -2, 0]
            sage: r = CFS((-2*x^3 + x^2 + 1)/(-2*x + 1))
            sage: r[0:5]              # handle ogf > 1
            [1, 2, 5, 8, 16]
            sage: r[-2]
            0
            sage: r = CFS((-2*x^3 + x^2 - x + 1)/(2*x^2 - 3*x + 1))
            sage: r[0:5]
            [1, 2, 5, 9, 17]
        """
    ...
    def ogf(self):
        """
        Return the ordinary generating function associated with the CFS.
        This is always a fraction of polynomials in the base ring.

        EXAMPLES::
        
            sage: R = CFS.from_recurrence([2],[1])
            sage: R.ogf()
            1/(-2*x + 1)
        """
    ...
    def series(self, n):
        """
        Return the Laurent power series associated with the CFS, with precision n.
        ...
        EXAMPLES::
            ...
            sage: r = CFS.from_recurrence([-1,2],[0,1])
            sage: r.series(4)
    ...
    def guess(sequence):
        """
        Return the minimal CFS that generates the sequence. Assume the first
        value has index 0.
        ...
        return CFS(numerator / denominator)
...
.. TODO::
        ...
        sage: CFS(x+x^2+x^3+x^4+x^5+O(x^6)) # not implemented
        ... x/(1-x)
    ...
    be given as extra parameter::
        ...
        sage: r = CFS(1/(1-y-x*y), x) # not tested
